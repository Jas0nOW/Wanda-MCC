...
async function getProcessLogs() {
  try {
    ...
    return { source: "Container Stdout", lines: stdout.split("\n").filter(Boolean) };
  } catch {
    return { source: "Error", lines: ["Konnte Logs nicht laden."] };
  }
}

export async function GET() {
  try {
    const logs = (await getOpenClawLogs()) ?? (await getProcessLogs());
